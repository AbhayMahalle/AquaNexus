...
import { Payment, Invoice, PaymentMethod } from '@/types/business';
...
export const DistributorPayments: React.FC = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [payRes, invRes, distRes] = await Promise.all([
        apiClient.getPayments(),
        apiClient.getInvoices(),
        apiClient.getDistributors(),
      ]);
      if (payRes.success) setPayments(payRes.data);
      if (distRes.success && distRes.data.length > 0) setDistributor(distRes.data[0]);
      if (invRes.success) {
  ...
  const handleMakePayment = async (e: React.FormEvent) => {
    ...
    const res = await apiClient.recordPayment({
      ...
      payerName: distributor?.name || 'Authorized Distributor Agency',
