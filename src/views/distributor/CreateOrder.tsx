...
export const CreateOrder: React.FC = () => {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (distRes.success && distRes.data.length > 0) {
        ...
        setDistributor(dist);
        if (dist.address) {
          setDeliveryAddress(dist.address);
        }
      }

      if (prodRes.success && prodRes.data.length > 0) {
        setProducts(prodRes.data);
        const selectedId = initialProductId && prodRes.data.some(p => p.id === initialProductId)
          ? initialProductId
          : prodRes.data[0].id;
        setItems([{ productId: selectedId, quantity: 50 }]);
      }
    }
    loadData();
  }, [initialProductId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await apiClient.createOrder({
      ...
      distributorName: distributor?.name || 'Distributor Agency',
      items: validItems,
            ...
            <CardContent className="space-y-4">
              {items.map((item, idx) => {
                const prodOpts = products.map(p => ({
                  label: `${p.name} (${formatCurrency(p.unitPrice)}/${p.unit})`,
                  value: p.id,
                }));
