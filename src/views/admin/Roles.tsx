...
const INITIAL_ROLES: RoleConfig[] = [
  {
    title: 'Admin', code: 'admin', users: 1, badge: 'primary',
    desc: 'Full unrestricted system configuration and audit access',
    permissions: ALL_PERMISSIONS.map(p => p.key),
  },
  {
    title: 'Operations Manager', code: 'manager', users: 1, badge: 'secondary',
  ...
  {
    title: 'Store Manager', code: 'store_manager', users: 1, badge: 'success',
    desc: 'Stock entry, raw material tracking, purchase requisitions',
    permissions: ['inventory.view', 'inventory.manage', 'reports.view'],
  },
  {
    title: 'Chief Accountant', code: 'accountant', users: 1, badge: 'info',
  ...
  {
    title: 'Distributor Agency', code: 'distributor', users: 1, badge: 'warning',
    ...
    async function loadRolesAndUsers() {
      try {
        const [rolesRes, usersRes] = await Promise.all([
          fetchApi<{ roles: any[] }>('/roles'),
          fetchApi<{ users: any[] }>('/users'),
        ]);

        if (rolesRes.success && rolesRes.data) {
          const rawRoles = rolesRes.data.roles || (Array.isArray(rolesRes.data) ? rolesRes.data : []);
          const rawUsers = usersRes.success && usersRes.data ? (usersRes.data.users || (Array.isArray(usersRes.data) ? usersRes.data : [])) : [];

          const badgeMap: Record<string, 'primary' | 'secondary' | 'success' | 'info' | 'warning'> = {
            ADMIN: 'primary',
            MANAGER: 'secondary',
          ...
          const mapped: RoleConfig[] = rawRoles.map((r: any) => {
            const userCount = rawUsers.filter((u: any) => (u.role?.name || u.userRoles?.[0]?.role?.name) === r.name).length;
            const perms: Permission[] = (r.rolePermissions || [])
              .map((rp: any) => rp.permission?.code)
              .filter((code: string) => ALL_PERMISSIONS.some(ap => ap.key === code));

            return {
              title: r.name.replace(/_/g, ' '),
              code: r.name.toLowerCase(),
              users: userCount,
              desc: r.description || `${r.name} role in AquaNexus`,
              badge: badgeMap[r.name] || 'primary',
              permissions: perms.length > 0 ? perms : (INITIAL_ROLES.find(ir => ir.code === r.name.toLowerCase())?.permissions || []),
            };
          });
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-all border ${
                    isEnabled
                      ? 'bg-[#E6EFF7] border-[#BAE0FD] text-[#0C3E6B]'
    ...
    <AuthGuard allowedRoles={['admin']}>
      <DashboardLayout>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {roles.filter(role => role.code.toLowerCase() !== 'admin').map((role) => (
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {role.permissions.slice(0, 4).map(p => (
                    <span key={p} className="px-2 py-0.5 text-[10px] font-mono bg-[#F5F8FB] text-[#172033] rounded-md border border-[#E2E8F0]">
                      {p}
                    </span>
                  ))}
                  {role.permissions.length > 4 && (
                    <span className="px-2 py-0.5 text-[10px] font-semibold text-[#64748B] bg-[#F5F8FB] rounded-md border border-[#E2E8F0]">
