  ...
  React.useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        const res = await apiRequest<any>(`/api/inventory?${queryParams.toString()}`);
        if (res.ok && res.data) {
          const rawItems = res.data.inventory || (Array.isArray(res.data) ? res.data : []);
          const mapped: InventoryItem[] = rawItems.map((inv: any) => {
            const qty = Number(inv.quantity) || 0;
            const reorder = Number(inv.reorderLevel ?? inv.product?.minimumStock ?? 0);
            ...
            return {
              ...
              status,
              lastMovement: inv.updatedAt ? new Date(inv.updatedAt).toLocaleDateString() : '--',
