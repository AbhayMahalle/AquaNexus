      ...
      >
        ...
        <div className="flex items-center justify-between h-16 px-5 border-b border-[#E5E5E5] bg-white">
          <Link to="/admin/dashboard" className="flex items-center gap-2.5 group">
            <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-[#FFF7ED] text-[#F97316] border border-[#FED7AA]/50 group-hover:scale-105 transition-transform">
              <Droplets className="w-5 h-5 fill-[#F97316]/20 text-[#F97316]" />
            </div>
            <div>
              <span className="text-base font-bold tracking-tight text-[#222222] block leading-tight">
                Aqua<span className="text-[#F97316]">Nexus</span>
              </span>
              <span className="text-[11px] text-[#999999] uppercase tracking-widest font-semibold block">
                Water Plant ERP
              </span>
            </div>
          </Link>
          ...
          {navSections.map((section, sIdx) => (
            <div key={sIdx} className="space-y-0.5">
              {section.sectionTitle && (
                <h4 className="px-3 text-[11px] font-bold uppercase tracking-wider text-[#999999] py-1">
                  {section.sectionTitle}
                </h4>
              )}
              {section.items.map((item) => {
                ...
                return (
                  <Link
                    ...
                    onClick={onMobileClose}
                    className={cn(
                      "flex items-center justify-between px-3 py-2 rounded-lg text-sm font-medium transition-colors duration-150 group border",
                      isActive
                  ...
                  >
                    ...
                    {item.badge && (
                      <span className={cn(
                        "px-1.5 py-0.5 text-[11px] font-bold rounded-md",
        ...
        <div className="p-3 border-t border-[#E5E5E5] bg-[#FAFAFA] text-center">
          <p className="text-[11px] text-[#999999] font-medium">AquaNexus Enterprise ERP</p>
        </div>
      </aside>
    </>
  );
