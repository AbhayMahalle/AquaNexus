      ...
      <table className="w-full text-left border-collapse min-w-full">
        <thead>
          <tr className="bg-[#F8F8F8] border-b border-[#E5E5E5]">
            {columns.map((col, idx) => {
              const colKey = String(col.key || col.accessorKey || col.accessor || idx);
              return (
              <th
                key={colKey}
            ...
            Array.from({ length: 4 }).map((_, rIdx) => (
              <tr key={`skel-row-${rIdx}`} className="animate-pulse">
                {columns.map((col, cIdx) => {
                  const colKey = String(col.key || col.accessorKey || col.accessor || cIdx);
                  return (
                  <td key={`skel-cell-${colKey}`} className="px-4 py-3.5 align-middle">
              ...
              return (
                ...
                >
                  {columns.map((col, cIdx) => {
                    ...
                    const valueKey = String(col.accessorKey || col.accessor || col.key || '');
                    return (
                    <td
                      key={`${key}-${colKey}`}
                      className={cn(
                    ...
                    >
                      {renderFn ? renderFn(row, rIdx) : (row as any)[valueKey]}
                    </td>
                  )})}
                </tr>
              );
