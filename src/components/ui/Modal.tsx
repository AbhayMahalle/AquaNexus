    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      ...
      <div 
        className={cn(
          ...
          maxWidth && typeof maxWidth === 'string' && maxWidth.startsWith('max-w') ? maxWidth : sizes[size as keyof typeof sizes] || sizes.md
        )}
        role="dialog"
        aria-modal="true"
      >
        <div className="flex items-start justify-between pb-3 sm:pb-4 border-b border-[#E5E5E5]">
          <div>
            ...
            {(description || subtitle) && <p className="mt-1 text-xs sm:text-sm text-[#666666]">{description || subtitle}</p>}
          </div>
          <Button
            variant="ghost"
            size="sm"
