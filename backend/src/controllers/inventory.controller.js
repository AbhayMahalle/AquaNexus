    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const transaction = await tx.stockTransaction.create({
        data: {
          ...
          quantity: qty,
          referenceType,
          referenceId: isUuid ? referenceId : null,
          remarks: (!isUuid && referenceId) ? `${remarks || ''} [Ref: ${referenceId}]`.trim() : remarks,
          createdBy: userId
        }
      });
...
/**
 * Get Goods Received Notes registry
 */
    ...
    const [records, total] = await Promise.all([
      prisma.goodsReceived.findMany({
        ...
        include: {
          ...
          production: { select: { id: true, productionNumber: true, batchNumber: true } },
