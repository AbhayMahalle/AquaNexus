...
const findRoleByIdOrName = async (prismaInstance, identifier) => {
  if (!identifier) return null;
  const allRoles = await prismaInstance.role.findMany();
  const search = identifier.toString().toLowerCase().replace(/\s+/g, '_');
...
const createUser = async (req, res) => {
  try {
    ...
    const role = await findRoleByIdOrName(prisma, roleId);
    if (!role) {
...
const updateUser = async (req, res) => {
  try {
    ...
    const user = await prisma.$transaction(async (transaction) => {
      if (roleId) {
        const role = await findRoleByIdOrName(transaction, roleId);
        if (!role) {
