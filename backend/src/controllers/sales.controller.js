...
const createSale = async (req, res) => {
  try {
    ...
    const ids = getAccessibleDistributorIds(req);
    if (!distributorId && ids && ids.length > 0) {
      distributorId = ids[0];
    }
    if (!saleNumber) {
...
const createReturn = async (req, res) => {
  try {
    ...
    const resolvedDistributorId =
      ids !== null
        ? req.body.distributorId && ids.includes(req.body.distributorId)
          ? req.body.distributorId
          : ids[0]
        : req.body.distributorId;

    const returnNumber =
      req.body.returnNumber || `RET-${Date.now().toString().slice(-6)}`;
    const returnDate = req.body.returnDate || new Date().toISOString();
    ...
    if (
      ...
      return sendError(
        res,
        "Distributor and at least one item are required",
        400,
      );
    ...
    if (
      ...
      return sendError(
        res,
        "Return items require positive quantities and a valid condition (GOOD or DAMAGED)",
        400,
      );
    const createdReturn = await prisma.$transaction(async (transaction) => {
      if (saleId) {
        ...
        if (!sale || sale.distributorId !== resolvedDistributorId) {
      ...
      const sanitizedItems = items.map((item) => ({
        ...
        ...(item.saleItemId ? { saleItemId: item.saleItemId } : {}),
      }));
      ...
      const record = await transaction.return.create({
        data: {
          ...
          reason: reason || null,
          createdBy: req.user.id,
          returnItems: { create: sanitizedItems },
        },
        include: { returnItems: { include: { product: true } }, distributor: true },
      });
